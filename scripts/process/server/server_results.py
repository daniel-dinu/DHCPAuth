...
def validate(file, row_number, current_total_time, current_auth_sign_time, current_auth_verify_time,
    ...
    try:
        ...
        print(constants.CONVERT_TOTAL_TIME_ERROR_MESSAGE.format(row_number, file, current_total_time))
        return False

    try:
        float(current_auth_sign_time)
    except ValueError:
        print(constants.CONVERT_AUTH_SIGN_TIME_ERROR_MESSAGE.format(row_number, file, current_auth_sign_time))
        return False

    try:
        float(current_auth_verify_time)
    except ValueError:
...
def process(file):
    """
    Process the given server results  CSV file
    :param file: The server results CSV file
    :return: The result of processing the given file: mean total time, mean auth sign time, mean auth verify time,
    sign packet length, sign auth length, verify packet length and verify auth length
    """
    ...
    with open(file, constants.CSV_FILE_ACCESS_MODE) as csv_file:
        ...
        for i, row in enumerate(rows):
            ...
            if 0 != i and constants.IGNORE_ROWS < i < total_rows - constants.IGNORE_ROWS:
                ...
                if valid:
                    total_time += float(row[constants.TOTAL_TIME_COLUMN_INDEX])

                    auth_sign_time += float(row[constants.AUTH_SIGN_TIME_COLUMN_INDEX])
                    auth_verify_time += float(row[constants.AUTH_VERIFY_TIME_COLUMN_INDEX])

                    sign_packet_length = int(row[constants.SIGN_PACKET_LENGTH_COLUMN_INDEX])
                    sign_auth_length = int(row[constants.SIGN_AUTH_LENGTH_COLUMN_INDEX])
                    verify_packet_length = int(row[constants.VERIFY_PACKET_LENGTH_COLUMN_INDEX])
                    verify_auth_length = int(row[constants.VERIFY_AUTH_LENGTH_COLUMN_INDEX])
    ...
    if 0 != count:
        ...
        auth_sign_time /= count
        auth_verify_time /= count
