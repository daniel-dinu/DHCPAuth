...
do
	echo "---> Generate $DSA_KEY_SIZE bits DSA PGP key pair $i <---"

	
	n=$((1000 + i)) 
	n=${n#1}
	
	private_key_file=$GENERATED_PRIVATE_KEY_FILE_PATH/priv_dsa_$n.key
	public_key_file=$GENERATED_PUBLIC_KEY_FILE_PATH/pub_dsa_$n.key
	owner_trust_file=$GENERATED_OWNER_TRUST_FILE_PATH/owner_dsa_$n.trust


	# Generate GPG script
	echo "Key-Type: DSA" > $GENRATE_KEY_SCRIPT
	echo "Key-Length: $DSA_KEY_SIZE" >> $GENRATE_KEY_SCRIPT
