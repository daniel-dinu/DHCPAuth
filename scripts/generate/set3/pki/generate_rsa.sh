...
do
	echo "---> Generate $RSA_KEY_SIZE bits RSA PKI key pair $i <---"


	n=$((1000 + i)) 
	n=${n#1}
	
	key_file=$GENERATED_KEY_FILE_PATH/key_rsa_$n.pem
	csr_file=$GENERATED_CSR_FILE_PATH/csr_rsa_$n.csr
	cert_file=$GENERATED_CERT_FILE_PATH/cert_rsa_$n.pem


	# Generate RSA private key
	openssl genrsa -aes256 -passout pass:$PASSWORD -out $key_file $RSA_KEY_SIZE -noout &>> $LOG_FILE
